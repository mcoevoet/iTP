<h3 align="center">Modifier un document : </h3>
...
if ((isset($_POST['titre']))&($_POST['titre'] != ""))
{
	...
	if($ok_mysql==1)
	{
		...
		$myquery = "update proc_db set titre='$titre' where ID='$ID'";
		mysql_query($myquery); 
		$myquery = "update proc_db set descr='$descr' where ID='$ID'";
		mysql_query($myquery); 
		$myquery = "update proc_db set cat='$cat' where ID='$ID'";
		mysql_query($myquery); 
		if ($change_file == "ok")
		{
			$myquery = "update proc_db set doc='$doc' where ID='$ID'";
			mysql_query($myquery);
		} 
		$myquery = "update proc_db set add_date='$add_date' where ID='$ID'";
		mysql_query($myquery); 
...
if ((isset($_POST['new_cat']))&($_POST['new_cat'] != ""))
{
	...
	$myquery = "insert INTO proc_cat (cat) VALUES ('$new_cat')" ;
	//echo "<br>", $myquery, "<br>" ;
	mysql_query($myquery); 
...
$ID = $_GET['ID'];
$Resultat = mysql_query("SELECT * FROM proc_db WHERE ID=$ID;");
...
<td>
<br>
<form action="index.php?toload=proc_modif" method="POST" enctype="multipart/form-data">
Titre :<br>
...
$Resultat = mysql_query("SELECT * FROM proc_cat ORDER BY cat;");
...
<form action="index.php?toload=proc_modif&ID=<?echo $ID?>" method="POST" enctype="multipart/form-data">
...
elseif($delete_record == "ok")
{
	...
	echo "<br><a href='index.php?toload=proc_admin_list'> Retour à la liste </a>";
