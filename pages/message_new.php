...
//	echo "dest : $dest_mail<br>";
	...
	$message_date = time();
	$read_item = "no";	
	$message_patient_data_rec = mysql_real_escape_string($_POST['message_patient_data']);
	...
	if($dest_type == "pers")
	{
		$myquery = "insert INTO message (exp, dest, date, subject, message, read_item, message_patient_data) VALUES ('$exp', '$dest_mail', '$message_date', '$subject', '$message_rec', '$read_item', '$message_patient_data_rec')";
//		echo $myquery;
		mysql_query($myquery);
//		echo "dest : ".$dest_mail;
...
//				echo "<br>filename : $file_name_upload <br>";
				$myquery = "insert INTO message (exp, dest, date, subject, message, read_item, message_patient_data) VALUES ('$exp', '$init', '$message_date', '$subject', '$message_rec', '$read_item', '$message_patient_data_rec')";
//				echo "$myquery<br>";
				mysql_query($myquery);
			}
...
<div align="center">
<!-- <form id="validate_form" action="index.php?toload=message_new" method="POST" name="new_mess_form" onsubmit="return verif_form()">-->
<form id="validate_form" action="index.php?toload=message_new" method="POST" name="new_mess_form">

Sujet : <input type="text" name="subject" id="special" class="validate[required]" data-prompt-position="bottomLeft:0,0"  <? if(isset($_GET['subject'])){echo "value=".$_GET['subject'];} ?> >
<br>
<?
...
if($_GET['response']=='yes')
{
	echo br2nl($_SESSION['message_reponse_patient_data']);
}
else
{
echo "Info patient";
...
if($_GET['response']=='yes')
{
	echo br2nl($_SESSION['message_reponse']);
}
?>
</textarea>
<br>
Destinataire :
<select name="dest" id="special2" class="validate[required]" onChange="mail_enable()" data-prompt-position="bottomLeft:0,0">
<!-- <select name="dest" id="special2" class="validate[required]"> -->

<?

//echo "<option value='".$dest_type.";".$init.";".$mail_dest_act.";".$mail_dest."'>".$_GET['dest']."</OPTION>";
if(isset($_GET['dest']))
{
echo "test";
