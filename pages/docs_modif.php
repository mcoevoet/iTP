...
if ((isset($_POST['titre']))&($_POST['titre'] != ""))
{
	...
	if($ok_mysql==1)
	{
		...
		$myquery = "update docs_db set titre='$titre' where ID='$ID'";
		mysql_query($myquery); 
		$myquery = "update docs_db set descr='$descr' where ID='$ID'";
		mysql_query($myquery); 
		$myquery = "update docs_db set cat='$cat' where ID='$ID'";
		mysql_query($myquery); 
		if ($change_file == "ok")
		{
			$myquery = "update docs_db set doc='$doc' where ID='$ID'";
			mysql_query($myquery);
		} 
		$myquery = "update docs_db set add_date='$add_date' where ID='$ID'";
		mysql_query($myquery); 
...
$ID = $_GET['ID'];
$Resultat = mysql_query("SELECT * FROM docs_db WHERE ID=$ID;");
...
<form action="index.php?toload=docs_modif" method="POST" enctype="multipart/form-data">
Titre :<br>
...
<form action="index.php?toload=docs_modif&ID=<?echo $ID?>" method="POST" enctype="multipart/form-data">
...
elseif($delete_record == "ok")
{
	...
	echo "<br><a href='index.php?toload=docs_admin_list'> Retour à la liste </a>";
